...
> [!IMPORTANT]
> The use of `$` or `>` as the first character on a line in any code block for a shell
> (commands etc.) is used to indicate the prompt with the command following. This is used
> to clarify a shell code block that contains commands and the (generally) expected
